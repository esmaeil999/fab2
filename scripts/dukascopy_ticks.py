...
class DukascopyBlocked(RuntimeError):
    """Raised when the server keeps rejecting requests (rate limit / IP block)."""


def fetch_hour(session: requests.Session, symbol: str, dt: datetime, retries: int = 5) -> bytes | None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if resp.status_code in (429, 503):
                raise requests.HTTPError(f"{resp.status_code} throttled", response=resp)
            resp.raise_for_status()
            return resp.content if resp.content else None
        except requests.RequestException as exc:
            if attempt == retries:
                print(f"  ! failed {url}: {exc}", file=sys.stderr)
                raise DukascopyBlocked(str(exc)) from exc
            # Exponential backoff: 2s, 4s, 8s, 16s
            wait = 2 ** attempt
            print(f"  ~ retry {attempt}/{retries - 1} in {wait}s ({exc})", file=sys.stderr)
            time.sleep(wait)
    return None
...
def is_market_closed(dt: datetime) -> bool:
    # FX market is closed from Friday ~22:00 UTC until Sunday ~22:00 UTC.
...
def download(symbol: str, start: datetime, end: datetime, delay: float = 0.5) -> pd.DataFrame:
    symbol = symbol.upper()
    ...
    all_ticks: list[dict] = []
    hours = [h for h in iter_hours(start, end) if not is_market_closed(h)]
    print(f"Downloading {symbol}: {len(hours)} hourly files from {start:%Y-%m-%d %H:00} to {end:%Y-%m-%d %H:00} UTC (weekends skipped)")

    for i, hour in enumerate(hours, 1):
        try:
            raw = fetch_hour(session, symbol, hour)
        except DukascopyBlocked:
            print(
                f"\nServer is rejecting requests after {i - 1} files. "
                "Saving what was downloaded so far. Try again later or increase --delay.",
                file=sys.stderr,
            )
            break
        if raw:
...
def main() -> None:
    ...
    parser.add_argument("--delay", type=float, default=0.5, help="Seconds to wait between requests (raise to 1-2 if you get 503s)")
